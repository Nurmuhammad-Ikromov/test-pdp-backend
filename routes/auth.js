...
router.post("/add", async (req, res) => {
  try {
    const { first_name, last_name, username, password, role, classId } =
      req.body;
    ...
    // Yangi user (teacher yoki student) yaratish
    const newUser = new User({
      ...
      role,
      password: hashedPassword,
      class: role === "student" ? classId : null, // faqat student bo‘lsa classId yozamiz
    });

    await newUser.save();

    // Agar student bo‘lsa, sinfga qo‘shib qo‘yamiz
    ...
    // Agar teacher bo‘lsa, sinfga qo‘shib qo‘yish logikasini ham yozib qo‘yish mumkin:
...
router.get("/profile", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    let grades = await ResponseExam.find({ who: user }).populate({
      path: "exam",
      select: "-encodedData",
    });
    return res.json({
